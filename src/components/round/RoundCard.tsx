...
export function RoundCard({ round, currentUserId, showStatus, hostName, noLink }: Props) {
  const to = `/rounds/${round.roundId}`
  const joined = currentUserId && round.memberIds?.includes(currentUserId)
  const resolvedHost = hostName ?? (round.createdBy === currentUserId ? 'You' : null)
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="min-w-0 flex-1 mr-3">
          ...
          <p className="text-sm text-gray-400 flex gap-1">
            <span className="text-green-400">{formatDate(round.date)}</span>
            {resolvedHost && <>
              <span className="text-gray-600">·</span>
              <span className="text-green-400">Host: {resolvedHost}</span>
            </>}
          </p>
        </div>
